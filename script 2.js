// ==========================================
//  JUNIOR+ 完全版 script.js (ランダム保存対応版)
// ==========================================

let quiz_data = { textbook_pages: {}, quizzes: [] };
let currentQuizList = [];
...
async function fetchAllData() {
    try {
        const [textbookRes, mondaiRes] = await Promise.all([fetch('./data.json'), fetch('./mondai.json')]);
...
function resumeQuiz() {
    ...
    if (!session) return;
    currentQuizList = session.quizIds.map(id => quiz_data.quizzes.find(q => q.id === id));
    currentQuizIndex = session.index;
    correctCount = session.correct;
    wrongQuestions = session.wrongIds.map(id => quiz_data.quizzes.find(q => q.id === id));
...
function finishQuiz() {
    ...
    saveHistory(rank, score, totalSeconds);
    quizScreen.classList.add('hidden');
    resultScreen.classList.remove('hidden');
}

function saveHistory(rank, score, time) {
    const historyData = {
        date: new Date().toLocaleString(),
        rank: rank, score: score, time: time,
        mode: isReviewMode ? '復習' : (pendingQuizMode === 'random' ? `ランダム(${pendingQuestionLimit}問)` : '通常'),
        wrongs: wrongQuestions.map(q => ({
            id: q.id, q_text: q.question, ans: q.correct_answer, hint: q.hint_text
        }))
    };
